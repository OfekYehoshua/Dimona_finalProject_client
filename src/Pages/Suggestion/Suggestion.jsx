 ...
 userLogged= JSON.parse(userLogged)
  ...
  const handleSubmit = async (e)=>{
    ...
    try {
      ...
      const suggestion = await axios.post(`${process.env.REACT_APP_API_URL}/api/suggestions/${userLogged._id}`,{title,phone:userLogged.phone,body,picture},config)
      if(suggestion){
    ...
    <div>
      ...
      <Form>
        <Form.Group className="mb-5 mt-3 ">
          ...
          <Form.Control type="text" placeholder="נושא"  onChange={(e)=>{setTitle(e.target.value)}} />
          <Form.Text className="text-muted"></Form.Text>
        </Form.Group>

        <Form.Group className="mb-5" controlId="exampleForm.ControlTextarea1">
          <Form.Label>טקסט</Form.Label>
          <Form.Control as="textarea" rows={6} placeholder=" אנא הכנס טקסט" onChange={(e)=>{setBody(e.target.value)}} />
        </Form.Group>

        <Form.Group controlId="formFile" className="mt-3">
        <Form.Label>יש אפשרות לעלות תמונה (לא חובה)</Form.Label>
        <br />
        <Form.Control type="file" onChange={(e)=>{setPic(e.target.files[0])}}  />
      </Form.Group>
      {picture&&
      <img src={picture} alt="" width={100} />  }
           <br />
        <Button onClick={(e)=>{handleSubmit(e)}} variant="primary" type="submit" className="custom-btn">
          שלח
        </Button>
      </Form>
    </div>
